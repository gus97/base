...
object CommbRule {
    ...
    def mergeMap(childMap: TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]]): TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]] = {
      for (x <- childMap) {
        if (fatherMap.contains(x._1)) {
          fatherMap += x._1 -> (fatherMap(x._1) ++ childMap(x._1))

        } else {
          fatherMap += x._1 -> childMap(x._1)
        }
      }
      fatherMap
    }
  ...
  def getKZS(ab: ArrayBuffer[Int]): Double = {
    ...
    if (tmp.count(_._1 >= 99) == 2) {
      cnt += 2.0
    } else if (tmp.count(_._1 == 100) == 1) {
      cnt += 1.0
    } else if (tmp.count(_._1 == 99) == 1) {
      cnt += 0.6
    }
    cnt += tmp.filter(_._1 == 15).getOrElse(15, 0) / 2.0
    cnt += tmp.count(_._2 == 4)
    cnt
  }
  ...
  def getCFT(ai: AI): TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]] = {
      ...
      for (y <- x._2) {
        ...
        if ((ai.r == 0 && y.size > ai.n1 && y.size > ai.n2 && x._1 != 11 && x._1 != 4) || (ai.r != 0 && y.size > ai.dz && x._1 != 11 && x._1 != 4)) {
          tm = tm.mergeMap(TreeMap(x._1 ->
            ArrayBuffer(y)))
        } else if (x._1 == 11 && y.exists(x => x >= ai.jpq.max)) {
          tm = tm.mergeMap(TreeMap(x._1 ->
            ArrayBuffer(
          ...
          if (y.head > 10) {
            ...
            val f = ld.filter(x => x > v).sliding(s, 1).count(x => x.last - x.head == s - 1)
          ...
          if (dz.nonEmpty && y.head > dz.keySet.max && ai.jpq.size > 3) {
            tm = tm.mergeMap(TreeMap(x._1 ->
              ArrayBuffer(y.filter(x => x >= dz
                .max._1))))
          } else if (dz.isEmpty) {
    ...
    for (x <- tm) {
      for (y <- x._2) {
        if (y.nonEmpty) {
          s = s.updated(x._1, s.getOrElse(x._1, ArrayBuffer()) ++ ArrayBuffer(y))
        }
      }
    }
    s
  ...
  def getCommbCFT(opt: TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]], jpq: ArrayBuffer[Int]): TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]] = {
    ...
    for (x <- opt) {
      for (y <- x._2) {
        ...
        if (x._1 == 11 && y.count(x => x >= jpq.max) > 0) {
          tm = tm.mergeMap(TreeMap(x._1 ->
            ArrayBuffer(
          ...
          if (dz.nonEmpty && y.head > dz.keySet.head) {
            tm = tm.mergeMap(TreeMap(x._1 ->
              ArrayBuffer(y.filter(x => x >= dz
                .max._1))))
          } else if (dz.isEmpty) {
  ...
  def getNCFT(ai: AI): TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]] = {
    ...
    for (x <- ai.opt if x._1 != 4 && x._1 != 1) {
      ...
      ncft = ncft.updated(x._1, ai.opt.getOrElse(x._1, ArrayBuffer())
        -- getCFT(ai).getOrElse(x._1, ArrayBuffer()))
    }
  ...
  def findSequenceByJPQ(a1: ArrayBuffer[Int], a2: ArrayBuffer[Int]): Boolean = {
    ...
    //记牌器剩余牌大于顺子长度
    if (tp.size >= a1.size) {
      for (_ <- a1.head to 14 if tp.size >= cnt && !b) {
  ...
  def threeOpt(ab: ArrayBuffer[Int], dqp: TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]],
    ...
    if (dqp.getOrElse(2, ArrayBuffer()).nonEmpty) {
      ab ++= dqp.getOrElse(2, ArrayBuffer()).flatten
      dqp(2).clear()
    }
    ...
    for (x <- dqp_c.getOrElse(33, ArrayBuffer())) {
      val len = x.distinct.size
      if (len <= f1.size) {
        val fs = f1.take(len)
        dqp_c = dqp_c.updated(33, dqp_c(33) - x)
        for (y <- fs) {
          dqp_c = dqp_c.updated(11, dqp_c.getOrElse(11, ArrayBuffer()) - ArrayBuffer(y))
        }
        dqp_c = dqp_c.updated(331, dqp_c.getOrElse(331, ArrayBuffer()) ++ ArrayBuffer(x ++ fs))
        ab --= fs
        f1 --= fs
      }
    }
    ...
    for (x <- dqp_c.getOrElse(33, ArrayBuffer())) {
      ...
      if (len <= f2.size) {
        ...
        dqp_c = dqp_c.updated(33, dqp_c(33) - x)
        for (y <- fs) {
          dqp_c = dqp_c.updated(2, dqp_c.getOrElse(2, ArrayBuffer()) - ArrayBuffer(y, y))
        }
        dqp_c = dqp_c.updated(332, dqp_c.getOrElse(332, ArrayBuffer()) ++ ArrayBuffer(x ++ (fs ++ fs).sortWith(_ < _)))
        ab --= fs
        ab --= fs
        f2 --= fs
      }
...
//    println("-----------------------------")
  ...
  def threeOptPassive(ai: AI, t: Int): ArrayBuffer[Int] = {
    ...
    if (t == 1) {
      val cd = ai.opt.flatMap(_._2).flatten.toBuffer
      var t3 = Buffer[Int]()
      var t2 = Buffer[Int]()
      ...
      for (x <- t3; y <- t2.filter(p => p != x).distinct.sortWith(_ < _) if flag) {

        if (ai.t.head < x) {

          val ab = ArrayBuffer[Int]()
          val tb = ai.opt.flatMap(_._2).flatten.toBuffer.clone

          tb.map(ab += _)
          ab --= ArrayBuffer(x, x, x, y)
          val oai = new AI
          val zp: TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]] = comb(ab, ai.jpq)
          oai.n1 = ai.n1
          oai.n2 = ai.n2
          oai.dz = ai.dz
          oai.jpq = ai.jpq
      ...
      for (x <- t3; y <- t2.filter(p => p != x).distinct.sortWith(_ < _) if flag) {
        if (ai.t.head < x) {
          ...
          val ab = ArrayBuffer[Int]()
          val tb = ai.opt.flatMap(_._2).flatten.toBuffer
          tb.map(ab += _)
          ab --= ArrayBuffer(x, x, x, y, y)
          val zp = comb(ab, ai.jpq)
    ...
    if (ops.opt.flatMap(_._2).nonEmpty) {
  ...
  def comb(ab: ArrayBuffer[Int], jpq: ArrayBuffer[Int]): TreeMap[Int, ArrayBuffer[ArrayBuffer[Int]]] = {
    var cd = ab.clone
    var d1 = rocket(cd)
      .mergeMap(boom(cd))
      .mergeMap(fly(cd))
    ...
    val b1 = dqp1.getOrElse(1, mutable.Buffer()).size + dqp1.getOrElse(4, mutable.Buffer()).size
    val b2 = dqp2.getOrElse(1, mutable.Buffer()).size + dqp2.getOrElse(4, mutable.Buffer()).size
    val b3 = dqp3.getOrElse(1, mutable.Buffer()).size + dqp3.getOrElse(4, mutable.Buffer()).size
    val b4 = dqp4.getOrElse(1, mutable.Buffer()).size + dqp4.getOrElse(4, mutable.Buffer()).size
    ...
    var opt = dqp(dqp.firstKey)

    if (opt.flatMap(_._2).flatten.size > 8) {
      ...
      val sd = opt.getOrElse(11, ArrayBuffer()).size

      var ho = 0
      if (sd > 0) {
        ho = opt(11).count(p => p.exists(p => p < 13))
      }

      if (opt.getOrElse(3, ArrayBuffer()).flatten.contains(15)) {

        opt = opt.updated(3, opt(3) - ArrayBuffer(15, 15, 15))
        for (x <- 1 to 3) {
          opt = opt.updated(11, opt(11) += ArrayBuffer(15))
          //opt = opt.updated(2, opt.get(2).get += ArrayBuffer(15, 15))
        }
      } else if (opt.getOrElse(2, ArrayBuffer()).flatten.contains(15) && ho >= 2) {

        opt = opt.updated(2, opt(2) - ArrayBuffer(15, 15))
        for (x <- 1 to 2) {
          opt = opt.updated(11, opt(11) += ArrayBuffer(15))
        }
      } else if (opt.getOrElse(31, ArrayBuffer()).flatten.contains(15)) {

        val o1 = opt(31).last.last
        opt = opt.updated(31, opt(31) - opt(31).last)

        for (x <- 1 to 3) {
          opt = opt.updated(11, opt(11) += ArrayBuffer(15))
        }
        opt = opt.updated(11, opt(11) += ArrayBuffer(o1))

      }
      else if (opt.getOrElse(32, ArrayBuffer()).flatten.contains(15)) {

        val o2 = opt(32).last.last

        opt = opt.updated(32, opt(32) - opt(32).last)
        for (x <- 1 to 3) {
          opt = opt.updated(11, opt(11) += ArrayBuffer(15))
        }
        opt = opt.updated(2, opt(2) += ArrayBuffer(o2,o2))
      }
    }

    if (opt.getOrElse(11, ArrayBuffer()).nonEmpty) {
      opt = opt.updated(11, opt.getOrElse(11, ArrayBuffer[ArrayBuffer[Int]]()).sortWith((x, y) => x.head < y.head))
    }
  ...
  def doubleForce(cd: ArrayBuffer[Int], c: AI): Seq[Int] = {
    ...
    var nt = Seq[Int]()
    for (x <- c.opt.flatMap(_._2.flatten)) {
      reAb += x
    }
    var cnt = 100
    //组牌后的最小套数
    ...
    for (x <- db) {
      if (bx < x) {
        val tmp = reAb.clone
        val reOpt = comb(tmp --= ArrayBuffer(x, x), c.jpq)
  ...
  def sigleForce(cd: ArrayBuffer[Int], c: AI): Seq[Int] = {
    ...
    for (x <- db) {
      if (bx < x) {
        val tmp = reAb.clone
        val reOpt = comb(tmp --= ArrayBuffer(x), c.jpq)
  ...
  def sequenceForce(cd: ArrayBuffer[Int], c: AI): Seq[Int] = {
    ...
    val slen = cd.size
    val ab = c.opt.flatMap(_._2.flatten).toSeq.distinct
    ...
    for (x <- 0 to dis.length - slen) {
      val t = dis.slice(x, x + slen)
      if (bx < t.head && t.last - t.head == slen - 1 && t.last < 15) {
        //重新组牌
        val tmp = reAb.clone()
        val reOpt = comb(tmp --= t, c.jpq)
        if (reOpt.flatMap(_._2).size < cnt) {
          minTs = reOpt
          nt = t
          cnt = reOpt.flatMap(_._2).size
  ...
  def threeForce(cd: ArrayBuffer[Int], c: AI): Seq[Int] = {
    ...
    for (x <- db) {
      if (bx < x) {
        val tmp = reAb.clone
        val reOpt = comb(tmp --= ArrayBuffer(x, x, x), c.jpq)
        if (reOpt.flatMap(_._2).size < cnt) {
          minTs = reOpt
  ...
  def sigleF(ai: AI): Seq[Int] = {
    ...
    val reAb = ArrayBuffer[Int]()
    var nt = Seq[Int]()
    for (x <- ai.opt.flatMap(_._2.flatten)) {
      reAb += x
    }
    ...
    for (x <- db) {
      if (bx < x) {
        val tmp = reAb.clone
        val reOpt = comb(tmp --= ArrayBuffer(x), ai.jpq)
  ...
  def sequenceF(ai: AI): Seq[Int] = {
    ...
    val slen = ai.t.size
    val ab = ai.opt.flatMap(_._2.flatten).toSeq.distinct
    val dis = ab.sortWith(_ < _)
    val reAb = ArrayBuffer[Int]()
    var nt = Seq[Int]()
    for (x <- ai.opt.flatMap(_._2.flatten)) {
  ...
  def doubleF(ai: AI): Seq[Int] = {
    ...
    for (x <- db) {
      if (bx < x) {
        val tmp = reAb.clone
        val reOpt = comb(tmp --= ArrayBuffer(x, x), ai.jpq)
  ...
  def sd1F(ai: AI): Seq[Int] = {
    ...
    val d = ai.opt.flatMap(_._2).flatten.toBuffer
  ...
  def sigleNewF(ai: AI): Seq[Int] = {
    ...
    val ncDNum = getNCFT(ai).getOrElse(2, ArrayBuffer()).size
    val ncSNum = getNCFT(ai).getOrElse(11, ArrayBuffer()).size
    val cc = ctrNumHitting(ai)

    val ts = ai.opt.flatMap(_._2).size

    //所有大于桌面牌的集合
    val ds = ai.opt.flatMap(_._2).flatten.toBuffer.filter(_ > t.head).distinct
    ...
    for (x <- ds) {
      val hp = ai.opt.flatMap(_._2).flatten.toBuffer.sortWith(_ < _).clone()
  ...
  def threeNewF(ai: AI): Seq[Int] = {
    ...
    for ((x, y) <- passAI if fg) {
      ...
      else if (ai.r == 0 && (ai.n1 == 2 || ai.n2 == 2) && ncNum <= getNCNum(y) + 1 && getNCFT(y).getOrElse(2, ArrayBuffer()).size == ncDNum) {
        ai.opt = y.opt
        hit = Seq(x, x, x)
      } //e)	若打出A后，非冲锋套非炸弹（火箭）的套牌套数【与原有非冲锋套非炸弹（火箭）的套牌套数相比】增加量<=1且非冲锋套单牌套数（与原有非冲锋套的单牌套数相比）增加量<1，则打出其中增加量最小标签最大的A，否则不出
      else if (ai.r == 0 && (ai.n1 == 1 || ai.n2 == 1) && ncNum <= getNCNum(y) + 1 && getNCFT(y).getOrElse(1, ArrayBuffer()).size == ncSNum) {
        ai.opt = y.opt
        hit = Seq(x, x, x)
      } else if (ai.r == 0 && ((ts == y.opt.flatMap(_._2).size) || ctrNumHitting(y) > (y.opt.flatMap(_._2).size - 2) ||
        (y.opt.flatMap(_._2).size - ctrNumHitting(y)) <= ts - cc || ctrNumHitting(y) >= 3)) {
  ...
  def seqNewF(ai: AI): Seq[Int] = {
    ...
    //所有大于桌面牌的集合
    val ds = ai.opt.flatMap(_._2).flatten.toBuffer.filter(x => x > t.head && x < 15).distinct.sortWith(_ < _)
    ...
    for (y <- ds) {
      ...
      val hp = ai.opt.flatMap(_._2).flatten.toBuffer.filter(x => x >= y && x < 15).distinct

      val hb = ai.opt.flatMap(_._2).flatten.toBuffer.sortWith(_ < _).clone()
    ...
    if (hit.nonEmpty) {
      for (x <- hit.head + 1 until hit.head + len) {
  ...
  def ctrNumHitting(ai: AI): Double = {
    ...
    val dp = ai.opt.flatMap(_._2).flatten.map((_, 1))
      .groupBy(_._1)
  ...
  def colourConvert(p: ArrayBuffer[Int], ai: AI): ArrayBuffer[String] = {
    ...
    if (p.isEmpty || p.head == -1) { return ArrayBuffer() }
  ...
  def optimizeHp(ai: AI): ArrayBuffer[Int] = {
    ...
    if (ai.opt.flatMap(_._2).size == 2 && ai.r != 0) {
      ...
      if (ai.opt.getOrElse(1, ArrayBuffer()).size == 1) {
        hit = ai.opt(1).head
        ai.opt.map { x => if (x._2.contains(hit)) x._2 -= hit }
        return hit
      } else if (ai.opt.getOrElse(4, ArrayBuffer()).nonEmpty) {

        var hasMaxB = ai.jpq.map((_, 1)).groupBy(_._1).map(x => (x._1, x._2.size)).exists(_._2 == 4) && ai.opt.getOrElse(4, ArrayBuffer()).nonEmpty && ai.jpq.map((_, 1)).groupBy(_._1).map(x => (x._1, x._2.size)).filter(_._2 == 4).max._1 < ai.opt(4).last.head

        if (!ai.jpq.map((_, 1)).groupBy(_._1).map(x => (x._1, x._2.size)).exists(_._2 == 4)) {
          hasMaxB = true
        }

        if (hasMaxB && !hasRock) {

          val h = ai.opt(4).filter(p => p.head > ai.t.head)
          if (h.nonEmpty) {
            hit = h.head
            ai.opt.map { x => if (x._2.contains(hit)) x._2 -= hit }
            return hit
          ...
          val hasB = ai.jpq.map((_, 1)).groupBy(_._1).map(x => (x._1, x._2.size)).exists(_._2 == 4)
          if (!hasRock && !hasB) {
            if (getCFT(ai).getOrElse(px, ArrayBuffer()).nonEmpty && getCFT(ai).flatMap(_._2).toBuffer.sortWith((x, y) => x.head > y.head).last.head > ai.t.head) {
            }
    ...
    if (getCNum(ai) + getNCNum(ai) == 0) {
      if (px != 4) {
        if (ai.opt.getOrElse(4, ArrayBuffer()).nonEmpty) {
          hit = ai.opt(4).head
          ai.opt.map { x => if (x._2.contains(hit)) x._2 -= hit }
        }
      }
...
//    val ab = ArrayBuffer[Int](12, 15, 3, 6, 4, 15, 3, 15, 5, 5, 12, 9, 11, 9, 5, 12, 6).sortWith(_<_)
//
//    println(ab)
//
//    val jpq = ArrayBuffer[Int](3, 3, 4, 4, 4, 5, 6, 6, 7, 7, 7, 7,r 8, 8, 8, 8, 9, 9, 10, 10, 10, 10, 11, 11, 11, 12, 13, 13, 13, 13, 14, 14, 14, 14, 15, 99, 100)
//
//    val s = comb(ab,jpq)
//
//    s.flatMap(_._2).foreach(println)
